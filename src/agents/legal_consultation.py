...
2. **必须标注**所引用的法条名称和条款号（格式：《民法典》第XXX条），不确定请明说
3. 如果涉及多个法律问题，请逐一解答
4. 给出具体的操作建议或维权步骤
5. 如果没有相关知识库法条可供参考，请基于你的法律知识回答，但明确告知用户"当前法律知识库有限，回答仅供参考"
6. 不得编造法条、案例或司法解释，不确定时必须诚实告知
7. 引用法条时直接写出原文内容，不要提及"片段X"、"第X条参考资料"等内部编号
...
class LegalConsultationAgent(BaseAgent):
    ...
    async def execute(
        ...
        logger.info(f"[AGENT] legal_consultation execute session={session_id}")
        # 1. RAG retrieval
    ...
    async def stream_execute(
        ...
        logger.info(f"[AGENT] legal_consultation stream session={session_id}")
        history_text = extract_history_from_context(context)
        try:
            law_results = await retrieve_legal(user_input, top_k=5, history_text=history_text)
        except Exception as e:
